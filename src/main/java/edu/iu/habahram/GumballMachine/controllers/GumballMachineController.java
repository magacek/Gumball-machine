...
public class GumballMachineController {
    ...
    @PutMapping("/refill")
    public GumballMachineRecord refill(@RequestBody TransitionRequest transitionRequest) {
        try {
            return gumballService.refill(transitionRequest.id(), transitionRequest.getCount());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
